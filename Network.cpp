...
double Network::train_on_batch(const std::vector<Matrix>& batch_inputs, 
    ...
    for (int i = 0; i < batch_size; ++i) {
        ...
        const Matrix& current_target = batch_targets[i];
    ...
    this->update_all_layer_parameters(learning_rate, batch_size);

    return total_batch_loss / static_cast<double>(batch_size); 
}
